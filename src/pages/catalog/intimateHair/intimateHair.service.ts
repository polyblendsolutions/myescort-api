...
export class IntimateHairService {
  ...
  constructor(
    @InjectModel('IntimateHair')
    private readonly intimateHairModel: Model<IntimateHair>,
   ...
   */
  async updateIntimateHairById(id: string, updateIntimateHairDto: UpdateIntimateHairDto): Promise<ResponsePayload> {
    try {
  ...
  async updateMultipleIntimateHairById(
    ...
    try {
      await this.intimateHairModel.updateMany({ _id: { $in: mIds } }, { $set: updateIntimateHairDto });
   ...
   * @param checkUsage
   */
  async deleteIntimateHairById(id: string, checkUsage: boolean): Promise<ResponsePayload> {
    let data;
    try {
  ...
  async deleteMultipleIntimateHairById(ids: string[], checkUsage: boolean): Promise<ResponsePayload> {
    try {
      const mIds = ids.map((m) => new ObjectId(m));
      ...
      if (checkUsage) {
        // Update Product
        await this.productModel.updateMany({}, { $pull: { intimateHairs: { $in: mIds } } });
      }
      return {
        success: true,
        message: 'Success',
