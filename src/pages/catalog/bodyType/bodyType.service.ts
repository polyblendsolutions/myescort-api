...
export class BodyTypeService {
   ...
   */
  async updateBodyTypeById(id: string, updateBodyTypeDto: UpdateBodyTypeDto): Promise<ResponsePayload> {
    try {
  ...
  async updateMultipleBodyTypeById(ids: string[], updateBodyTypeDto: UpdateBodyTypeDto): Promise<ResponsePayload> {
    ...
    try {
      await this.bodyTypeModel.updateMany({ _id: { $in: mIds } }, { $set: updateBodyTypeDto });
   ...
   * @param checkUsage
   */
  async deleteBodyTypeById(id: string, checkUsage: boolean): Promise<ResponsePayload> {
    let data;
    try {
  ...
  async deleteMultipleBodyTypeById(ids: string[], checkUsage: boolean): Promise<ResponsePayload> {
    try {
      const mIds = ids.map((m) => new ObjectId(m));
      ...
      if (checkUsage) {
        // Update Product
        await this.productModel.updateMany({}, { $pull: { bodyTypes: { $in: mIds } } });
      }
      return {
        success: true,
        message: 'Success',
