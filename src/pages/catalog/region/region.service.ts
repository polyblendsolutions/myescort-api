...
export class RegionService {
  ...
  async insertManyRegion(addRegionsDto: AddRegionDto[], optionRegionDto: OptionRegionDto): Promise<ResponsePayload> {
  ...
  async getAllRegions(filterRegionDto: FilterAndPaginationRegionDto, searchQuery?: string): Promise<ResponsePayload> {
   ...
   */
  async updateRegionById(id: string, updateRegionDto: UpdateRegionDto): Promise<ResponsePayload> {
    try {
  ...
  async updateMultipleRegionById(ids: string[], updateRegionDto: UpdateRegionDto): Promise<ResponsePayload> {
    ...
    try {
      await this.regionModel.updateMany({ _id: { $in: mIds } }, { $set: updateRegionDto });
   ...
   * @param checkUsage
   */
  async deleteRegionById(id: string, checkUsage: boolean): Promise<ResponsePayload> {
    let data;
    try {
  ...
  async deleteMultipleRegionById(ids: string[], checkUsage: boolean): Promise<ResponsePayload> {
    try {
      const mIds = ids.map((m) => new ObjectId(m));
      ...
      if (checkUsage) {
        // Update Product
        await this.productModel.updateMany({}, { $pull: { regions: { $in: mIds } } });
      }
      return {
        success: true,
        message: 'Success',
