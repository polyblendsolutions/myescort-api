...
export class HairColorService {
   ...
   */
  async updateHairColorById(id: string, updateHairColorDto: UpdateHairColorDto): Promise<ResponsePayload> {
    try {
  ...
  async updateMultipleHairColorById(ids: string[], updateHairColorDto: UpdateHairColorDto): Promise<ResponsePayload> {
    ...
    try {
      await this.hairColorModel.updateMany({ _id: { $in: mIds } }, { $set: updateHairColorDto });
   ...
   * @param checkUsage
   */
  async deleteHairColorById(id: string, checkUsage: boolean): Promise<ResponsePayload> {
    let data;
    try {
  ...
  async deleteMultipleHairColorById(ids: string[], checkUsage: boolean): Promise<ResponsePayload> {
    try {
      const mIds = ids.map((m) => new ObjectId(m));
      ...
      if (checkUsage) {
        // Update Product
        await this.productModel.updateMany({}, { $pull: { hairColors: { $in: mIds } } });
      }
      return {
        success: true,
        message: 'Success',
