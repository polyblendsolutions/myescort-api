...
export class AppController {
  ...
  @Get('backup')
  async backupDatabase(@Query('password') password: string): Promise<ResponsePayload> {
    return await this.appService.backupDatabase(password);
  }

  @Version(VERSION_NEUTRAL)
  @Get('restore')
  async restoreDatabase(@Query('password') password: string): Promise<ResponsePayload> {
